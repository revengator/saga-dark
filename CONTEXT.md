...
- **PT3 audition counter (debug aid, 2026-06-19).** A throwaway variant of `test-music.asm` can draw the current PT3 **pattern number** on-screen (top-left) so a module is auditioned section-by-section: the IM2 handler keeps a row counter (advances every `speed` frames; `pattern = row>>6` for 64-row patterns, wrap at `npat*64`), splits to decimal, and draws 8×8 glyphs from the **48K-ROM font** (`0x3C00 + char*8`, line stride `+0x100`) to `0x4000`, with a couple of bright-white attrs at `0x5800` (boot zeroes the screen → ink invisible otherwise). The user verifies by ear and reports "pattern N is wrong" → maps straight to the module's rows. Companion: slice the melody at its silences into numbered segments, audition each looping solo. Keep per-experiment players/`.sna` out of git. See [[ay-music-audition-pattern-counter]].
...
- **(E) 🔥 Unified engine refactor on `platformer/` — primary active thread, 2026-05-20+.** Goal: turn the platformer from "two phases swapped via LDIR" into a single continuous game (Dark 1 + Dark 2 levels flow into each other) and the host of the missing end-game content (Level 5 Kamuir / Book / ending). The `_in_progress/128k-plus2-extended/` folder was retired into the platformer on this date; technique catalogue moved to `remakes/_in_progress/128k-plus2-platformer-v2/IDEAS.md`.
  ...
  - **E7 — Level 5 / Kamuir's Fortress + final boss + Book pickup + victory cutscene.** The endgame content. **⏸ PARKED 2026-06-22** — dev paused; nothing wired into the build. Music groundwork ready: two committed theme candidates (`kamuir.pt3` / `barbarian.pt3`) + their compose scripts in `tools/`, all regenerable. To resume: pick a theme, add a `song_level5_data` INCBIN + one `music_play_track` call at the Level 5 entry (the per-level-music hook is already extensible — see the F-thread note in the header).
