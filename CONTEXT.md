...
8. **Distribution: `.tap` / `.trd` / `.dsk` / `.nex` builds + JSSpeccy embed (`index.html`)** — no engine change, broadens reach. ⚙️ Web menu DONE 2026-06-16: root `index.html` now lists all 8 committed snapshots in two groups (Remakes +2 128K: platformer normal+cheat, dragon, film · Originals 48K: dark1/2/3 + film.z80); JSSpeccy auto-switches 48K↔128K from each snapshot's model (`loadSnapshot`→`setMachineType`). Served via GitHub Pages. 48K-originals cheat `.sna` not committed (needs sjasmplus build).
