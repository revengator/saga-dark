...
ROUT	XOR A
ROUT_A0
;Saga Dark patch: optional per-channel volume scaler so keyboard SFX can stay
;audible over the music. DISABLED for the film — the cutscene has no keyboard
;SFX to duck under, and the /2 was hurting the music twice over: it halved the
;amplitude AND crushed the per-note dynamics, since the nibble truncates on
;shift (vol 15 and 14 both -> 7, 13/12 -> 6, 11/10 -> 5 — the accents vanished).
;The loop below now passes the 4-bit volume nibble through unchanged (full
;volume), still preserving bit 4 (envelope-enable) of AmplA/B/C and the XOR A
;reg-select restore. To re-enable ducking, uncomment the `SRL A` (one per /2).
	LD HL,AYREGS+AmplA
	LD B,3
ROUTVH	LD A,(HL)
	AND #10
	...
	LD A,(HL)
	AND #0F
	;SRL A          ; halving disabled — see the patch note above (full volume)
	OR C
	LD (HL),A
	INC HL
	DJNZ ROUTVH
