...
ROUT	XOR A
ROUT_A0
;Saga Dark patch: scale channel volume down so keyboard SFX stays audible.
;Each SRL A shifts the 4-bit volume nibble right by 1 (halve). Two SRL A
;in a row → volume / 4. Preserves bit 4 (envelope-enable) of AmplA/B/C;
;runs once per tick. Path covers PLAY fast-path (JR Z,ROUT_A0 line 914),
;env-step path (line 916), MUTE (line 94, halving zero is zero), and
;INIT-end (line 331, AYREGS already zeroed). XOR A at end restores
;reg-select. Tune the divisor by adding/removing SRL A pairs.
	LD HL,AYREGS+AmplA
	LD B,3
ROUTVH	LD A,(HL)
	AND #10
