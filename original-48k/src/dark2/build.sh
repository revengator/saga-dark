#!/usr/bin/env bash
# Compila dark2 a .bin y .sna ejecutable.
# Uso:
#   ./build.sh           — build normal (byte-perfect contra dark2.sna original)
...
SNA_ORIG="$SCRIPT_DIR/../../snapshots/dark2/dark2.sna"
...
build_one() {
    ...
    cd "$SCRIPT_DIR"
    if [ -n "$flag" ]; then
        sjasmplus -D"$flag" --raw="$bin" dark2.asm > /dev/null
    else
        sjasmplus --raw="$bin" dark2.asm > /dev/null
    fi
...
case "${1:-normal}" in
    normal)
        ...
        ;;
    cheat)
        build_one "dark2_cheat" "INFINITE_ENERGY=1"
        ;;
    all)
        build_one "dark2" ""
        build_one "dark2_cheat" "INFINITE_ENERGY=1"
